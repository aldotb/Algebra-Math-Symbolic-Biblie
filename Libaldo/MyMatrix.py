...
class MyMat:
    def __init__(self, *args,kshow=True):
        kres=args[0]
        self.type='M'
        if type(kres)==list and  type(kres[0])!=list:
            
                M=Matrix(kres)
                M=M.T
                self.type='V'
        elif type(kres)==list and  type(kres[0])==list:     
                M=Matrix(kres)
        else:    
            try: 
                self.type='M'
    ...
    def angle(self,*args):
        if self.type=='V':
            ...
            for data in args:
                if data != 'float':
                    if p1=='':
                        p1=vecv[vecs.index(data)]
                    else:
                        p2=vecv[vecs.index(data)]
            
            kres=atan(cfrac(p1,p2))
            if 'float' in args:
                kres=float(kres)
            return kres
