...
vecxsym=symbols('x:26')
vecxsym=vecxsym[1:-1]
vecysym=symbols('y:26')
vecysym=vecysym[1:-1]
...
def getdataMat(expr):
    if type(expr)==MyMat:
        return expr.M
    elif type(expr)==MutableDenseMatrix:
        return expr
    else:
        return ''
...
class MyMat:
    ...
    def __call__(self,*args,**kwargs):
        ...
        if len(args)==0 and len(kwargs)==0:
            return  Matrix(parse_expr(str(self)))
        elif len(args)==1:
            return M[args[0]]
        elif len(args)==2:
    ...
    def set(self,*args,**kwargs):
        if len(args)==3:
            if Is_Znumber(args[0]) and Is_Znumber(args[1]):
                MM=self.M
                MM[args[0],args[1]]=args[2]
                self.M=MM
        else:            
            if len(args)>0 and len(kwargs)==0:
                MM=self.M
...
def createMatrix(*args,kshow=True):
    ...
    nrow=args[0]
    ncol=args[1]
    vecop=['b','c','x','y','simetric','asimetric']
...
def msolve(obj,*args):
    if 'cero' in args:
        ...
        vecee=[]
        for i in vecr:
            vecee.append(MyEq(i,kshow=False))
        for i in args:
            if i!='cero':
                vecee.append(i)
        return  simplesolve(*vecee )
    else:    
        ...
        if vecv==[]:
            vecv=[1]*Lx
        if vecr==[]:
            vecr=[0 for i in range(Ly)]
    
          
        for i in range(Ly):
            Vecp=0
            for j in range(Lx):
                Vecp=Vecp+M[i,j]*vecv[j]  
            vecexpr.append(Vecp)
            
        try:
            kres=[aa-bb for aa,bb in zip(vecexpr,vecr)]
            return  simplesolve(kres )
        except:    
            return vecexpr
...
class MyVecx(MyMat):
    def __init__(self, *args,kshow=True):
        ...
        if kshow:
            pstr=self.name+'=' 
            display(Math(pstr+latex(self.M)))
