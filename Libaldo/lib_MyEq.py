...
class MyEq:
    def __init__(self, ksym, kname='', var=x,var1=y,var2=t,  varx=x, vary=y, varz=z, kp=False, kope='', kshow=True, ktype='P',
        ...
        if kshow:
            if ktype=='diff' or ktype=='diff2':
                ...
                self.ksym=fres    
                if kname=='' and self.type=='D':
                    self.name='d'+self.name
                sR = self.name + ' ='
                kres=self.ksym*self.diffvar
    ...
    def __call__(self,*args,kshow=True, **kwargs):
        ...
        if len(args)==1 and len(kwargs)==0:
            ...
            try:
                ...
                if type(Obj)==Point2D:
                    P=list(Obj)
                    ksym=ksym.subs(x,P[0])
                    ksym=ksym.subs(y,P[1])
                ...
                if type(Obj)==Point3D:
                    ...
                    ksym=ksym.subs(x,P[0])
                    ksym=ksym.subs(y,P[1])
                    ksym=ksym.subs(z,P[2])
 
                    return ksym 
            except:
                var=self.var
                valor=args[0]
                ksym=ksym.subs(var,valor)
    ...
    def set(self,*args,kshow=True,**kwargs):
        ksym=unisymbols(self.ksym)
         
            
        for i in args:
            if type(i)==MyEq:
                if i.name=='α':
                    ksym=unisymbols(ksym.subs(alpha,i.ksym))
                     
                else:
                    sname=i.name
                    sval=unisymbols(i.ksym)
                    ksym=ksym.subs(sname,sval)
        self.ksym=ksym         
        if len(kwargs) > 0:
    ...
    def clone(self,kname=''):
        if kname!='':
            ee=MyEq(self.ksym,kname=kname,var=self.var)
            return ee
        else:
            ee=MyEq(self.ksym,kname=self.name,var=self.var,kshow=False)
            return ee
    ...
    ##############################################
    ##  Transformacion
    def square2polar(self):
        kres=self.ksym
        return square2polar(kres)

        
    def squarecompletexy(self,*args,var1=x,var2=y,kshow=True):
        kres=self.ksym
        if 'parts' in args:
            return squarecompletexy(kres,'parts',var1=var1,var2=var2)
        else:
            kres=squarecompletexy(kres,var1=var1,var2=var2)
            self.ksym=kres
            if kshow:
                self.s()
    def square2parametric(self,*args):
        xx,yy=squarexy2Polar(self.ksym,kshow=False)
        if len(args)==0:
            return xx,yy
        else:
            e1=MyEq(xx,kname=args[0])
            e2=MyEq(yy,kname=args[1])
            return e1,e2

        
            
    def squarefill(self,*args):
        kres=self.ksym
         
            
        kres=squarefill(kres,*args)
    ...
    def subs3(self,val1,val2,kshow=True):
        if type(val1)==MyEq:
            val1=val1.ksym
        if type(val2)==MyEq:
            val2=val2.ksym   
    ...
    def simplifysigno(self,kshow=True):
        kres = self.ksym
        kres=simplifysigno(kres)
        self.ksym=kres
        if kshow:
            self.s()
    def simplifysum(self,kshow=True):
        kres = self.ksym
        kres=simplifysum(kres)
        self.ksym=kres
        if kshow:
            self.s()
    def simplifymul(self,kshow=True):
        kres = self.ksym
        kres=simplifymul(kres)
        self.ksym=kres
        if kshow:
            self.s()       
    ...
    def short(self):
        try:
            ksym=self.ksym
            skres=numbershort(ksym)
            kname=self.name
            display(Math(kname+'='+skres))
    ...
    def rpart(self):
        return rpart(simplify(expand(self.ksym)))
    def ipart(self):
        return ipart(simplify(expand(self.ksym)))
    def conjug(self):
        return conjugada(simplify(expand(self.ksym)))
    def imodulo(self):
        rp=rpart(self.ksym)
        ip=ipart(self.ksym)
        imod=sqrt(rp*rp+ip*ip)
        return imod
    def arg(self):
        return arg(simplify(expand(self.ksym)))

    def ipolar(self):
        return self.imodulo(),self.arg()
    def itrigo(self):
        r,alpha=self.ipolar()
        return r*(cos(alpha)+sin(alpha)*I)
    ...
    def lsimplify(self, kupdate=True, kshow=True, kope=''):
        ...
        if kshow:
            self.s()        
    def lcombine(self, kupdate=True, kshow=True, kope=''):
        kres = self.ksym
        kres = lcombine(kres)
        kres = opemat(kres, kope=kope)
        if kupdate:
    ...
    def summ(self,x1,x2,kshow=True):
        kres=self.ksym
        var=self.var
        kres=summation(kres, (var, x1, x2))
        self.ksym=kres
        if kshow:
            self.s()
    ...
    def rsimplify(self,kname='',kshow=True):
        ...
        if kname!='':
            ...
            self.ksym=kres
            self.s()        
    def sfactor(self,kname='',kshow=True):
    ...
    def tsimplify(self,*args,kshow=True, kope=''):
        kres = self.ksym
        kres = trigsimp(kres)
        kres = opemat(kres, kope=kope)
        self.ksym=kres
    ...
    def lexpand(self, force=True,kshow=True):
        kres = self.ksym
        kres = lexpand(kres,force=force)
        self.ksym=kres 
        if kshow:
            self.s() 
    def lexponent(self):
    ...
    def positivexpo(self,kname='',kshow=True):
        ksym = self.ksym
        kres=positivexpo(ksym)
        if self.get_deno()!=1:
            kres=cfrac(positivexpo(self.get_nume()),positivexpo(self.get_deno()))
    ...
    def lexponent(self,kname='',kshow=True):
        ...
        if kname!='':
            ...
            self.ksym=kres
            self.s()
    def log(self,*args, kshow=True, kope=''):
        kres = self.ksym
        if len(args)==1:
            kres = log(kres,args[0])
        else:
            kres = log(kres,base)
        try:
            kres=lsimplify(kres)
    ...
    def cloneAbs(self):
        ...
        kres=self.ksym
        gname=self.name
        return cloneAbs(kres,pname=gname,var=fvar)
    ...
    def solve(self,*args,kshow=True,**kwargs):
        ...
        if 'positive' in args:
            kres2=[data for data in kres if signo(data)!=-1]
            kres=kres2
        if 'noimg' in args:
        ...
        if svar!='':
            if len(kres)>1:
                ...
                vece=[]
                for data in kres:
                    vece.append(MyEq(data,svar+'_'+str(cc),kshow=kshow))
                    cc+=1
                return vece
            else:
                return MyEq(kres[0],svar,kshow=kshow)
        else:
            if len(kres)==1 and not 'list' in args:
                return kres[0]
            else:
                return kres
    ...
    def solve(self,var,*args,**kwargs):
        svar=''
        expr=self.ksym
        expr=real_subs(expr,**kwargs)
        edone=False
        ...
        if edone:
            if len(kres)==1:
                ...
                evec=[]
                for i in range(1,len(kres)+1):
                    evec.append(MyEq(kres[i-1],svar+str(i)))
    ...
    def factorize(self,ksym,kshow=True):
       ...
       #return x**3*(3*x**4+2)
        ...
        if kshow:
            self.s()
    def factoriza(self,*args,kshow=True):
    ...
    def domain(self,*args):
        ...
        var=self.var
        if len(args)==0:
            kres=continuous_domain(expr, var,S.Reals)
        else:
            sexpr=args[0]
            s1=sexpr[0]
            s2=sexpr[-1]
            ...
            if s1=='[' and s2==']':
                kres= continuous_domain(expr, var,Interval(p1,p2))
            elif s1=='<' and s2==']':
                kres= continuous_domain(expr, var,Interval.Lopen(p1,p2))
            elif s1=='[' and s2=='>':
                kres= continuous_domain(expr, var,Interval.Ropen(p1,p2))
            else:
                kres= continuous_domain(expr, var,Interval.open(p1,p2))
        try:
            return eval(kres)
        except:
            return kres    
    def range(self,*args):
        ...
        var=self.var
        if len(args)==0:
            kres=function_range(expr, var,S.Reals)
        else:
            sexpr=args[0]
            s1=sexpr[0]
            s2=sexpr[-1]
            ...
            if s1=='[' and s2==']':
                kres= function_range(expr, var,Interval(p1,p2))
            elif s1=='<' and s2==']':
                kres= function_range(expr, var,Interval.Lopen(p1,p2))
            elif s1=='[' and s2=='>':
                kres= function_range(expr, var,Interval.Ropen(p1,p2))
            else:
                kres= function_range(expr, var,Interval.open(p1,p2))
        try:
            return eval(kres)
        except:
            return kres     
    ...
    def rotate2D(self,*args):
        ...
        X=X.subs(alpha,angle)
        Y=Y.subs(alpha,angle)
        ee=MyEq(self.ksym,'ee',kshow=False)
        ee.set(kshow=False,x=X,y=Y)
        kres=simplify(expand(ee.ksym))
        kres=kres.subs(x1,x)
        kres=kres.subs(y1,y)
        try:
            kres=reducecero(kres)
        except:
            pass
        if kname!='':
    ...
    #  Integral
    #######################    
    def simplesolve(self,*args,**kwargs):
        ops=['noshow'] 
        expr=self.ksym
        edone=False
        var=args[0]
    ...
    def Derivative(self,*args):
        ...
        for data in args:
            if type(data)==Symbol:
                ...
                pass    
        kres=Derivative(self.ksym,*secd)
        if kname!='':
            return MyEq(kres,kname)
        else:
            return kres
    ...
    def parcialdiff(self,*args,kshow=True):
        kname=''
        for i in args:
            if 'd' in i or '/' in i:
                svar=i
                ...
                kname=i
        vecvar=cleandfs(svar)  
        kres=self.ksym
        p1=diff(kres,vecvar[0])
        if len(vecvar)==1:
            p2=1
        else:    
            p2=diff(kres,vecvar[1])
        kres=simplify(p1/p2)
        if kname!='':
            return MyEq(kres,kname=kname,kshow=kshow)
        else:
            return kres
        

        
    def diff(self,*args,func='',kshow=True,**kwargs):
        kname=''
    ...
    def eQdiff(self,*args,kshow=True):
        kname=''
        var=self.var
        vecd=[]
        for i in args:
        ...
        if kname=='':
            kname='d'+self.name
        ee=MyEq(kres,kname=kname,var=var,ktype='D')
        return ee
    ...
    def sin2cos(self, ang=alpha, kshow=True):
        kres=self.ksym
        kres=sin2cos(kres,ang=ang)
        self.ksym=kres
        if kshow:
            self.s()
         

    def cos2sin(self, ang=alpha, kshow=True):
        kres=self.ksym
        kres=cos2sin(kres,ang=ang)
        self.ksym=kres
        if kshow:
            self.s()
    def tan2sincos(self, ang=alpha, kshow=True):
        kres=self.ksym
    ...
    def maximiza(self,*args):
        ...
        ksolu=solve(diff(kres,kvar),kvar)
        if type(ksolu)==list and len(ksolu)==1:
            ksolu=ksolu[0]
        if eqdone:
            return MyEq(ksolu,eqname)
        else:
            return ksolu
        
    def maximize(self,*args):
        return self.maximiza(*args)
...
def squarexy2Polar(obj,var=t,kshow=True):
    expr=obj2expr(obj)
    mm=expr.args
...
def locatefunc(expr,sfunc):
    sexpr=str(expr) 
    p=sexpr.find(sfunc)
    return p
def function_pos(expr,sfunc):
    p1=locatefunc(expr,'Abs(')
    p2,p3=find_simetric_end_par(expr,p1)
    return p2-3,p3

def get_spartfunc(expr,p1,p2):
    sexpr=str(expr)
    return sexpr[p1:p2+1]
...
def get_spartfunc(expr,p1,p2):
    sexpr=str(expr)
    return sexpr[p1:p2+1]            
def find_function(expr,sfunc):
    p1,p2=function_pos(expr,sfunc)
    spart=get_spartfunc(expr,p1,p2)
    return spart

def desdoblaAbs(expr):
    vec=[]
    sexpr=str(expr)
    pabs=find_function(expr,'Abs(')
    insid=insideabs(pabs)
    sinsid=str(insid)
    vec.append(sexpr.replace(pabs,sinsid))
    vec.append(sexpr.replace(pabs,'(-1*('+sinsid+'))'))
    vres=[parse_expr(i) for i in vec]
    return vres

def cloneAbs(obj,pname='',var=x):
    cc=1
    kres=obj 
    vecab=desdoblaAbs(kres)
    vecee=[]
    kvar=var
    for i in range(len(vecab)):
        vecee.append(MyEq(vecab[i],kname=pname+str(cc),var=kvar))
        cc=cc+1
    return vecee
...
def eqSolve(*args,**kwargs):
    vops=[]
    vvar=[]
    vexpr=[]
    edone=False
    ops=['float','noshow','noimg','positive']
    for data in args:
        if type(data)==str:
            if not data in ops:
                data=symbols(data)
    ...
    if len(kwargs)>0:
        nvecexp=[]
        for data in vexpr:
            rdata=real_subs(data,**kwargs)
        nvecexp.append(rdata)
        vexpr=nvecexp
    if len(vexpr)==1:
         kres=solve(vexpr[0],vvar[0])
    else:
        kres=solve(vexpr,vvar)
    sdone=True   
    if  'noshow' in vops:
        sdone=False
    veceq=[]    
    if len(kres)>1:
       sname=[str(vvar[0])+str(ss) for ss in range(1,len(kres)+1) ]
       for name,vkres in zip(sname,kres):
           ee=MyEq(vkres,name,kshow=sdone)
           veceq.append(ee) 
    else:
        veceq=MyEq(kres[0],vvar[0],kshow=sdone)
        kres=[0]
    if edone:
        if len(veceq)==1:
            return veceq[0]
